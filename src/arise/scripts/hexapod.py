        ...
        match self.gait:
            ...
            case "tripod":
                ...
                if target_speed == 0:  
                    ...
                    if abs((self.t % 1) - 0.50) < 0.001:
                        self.current_direction = direction

                    if self.current_direction < 0:
                        self.current_direction = self.current_direction + 2*np.pi
                    P0 = np.array([np.sin(self.current_direction) * -step_length/2, 
                                np.cos(self.current_direction) * -step_length/2, 
                                0]) # start of lift
                    P1 = np.array([0, 0, -step_height]) # control
                    P2 = np.array([np.sin(self.current_direction) * step_length/2, 
                                np.cos(self.current_direction) * step_length/2, 
                                0]) # end of lift
